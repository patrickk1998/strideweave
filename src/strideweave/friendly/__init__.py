"""Ergonomic helpers for creating and inspecting strideweave tensors.

``strideweave.friendly`` wraps the composable primitives — carriers,
...
over the CPU carrier, compact layout builders, scalar reductions, and value
extraction. The package is submodule-only — import it explicitly, e.g.
``import strideweave.friendly as F`` — so the top-level ``strideweave`` namespace
