...
public:
    ...
    py::object dtype() const override { return cpu_dtype_object(dtype_); }

    CpuDType cpu_dtype() const { return dtype_; }
...
inline CPU& cpu_carrier_from_tensor(py::handle tensor, const char* name) {
    ...
    CPU& cpu_carrier = py::cast<CPU&>(carrier);
    if (cpu_carrier.is_released()) {
        throw std::runtime_error(std::string(name) + " carrier is released");
    }
    return cpu_carrier;
}
...
struct CpuTensorView {
    ...
    void write_float_logical(Index key, float value) const {
        const Index layout_index = cache->index_logical(key);
        carrier->unchecked_element<float>(offset + layout_index) = value;
    }
    ...
    std::int32_t read_int_expanded(const Index* key, std::size_t size) const {
        const Index layout_index = cache->index_expanded(key, size);
        return carrier->unchecked_element<std::int32_t>(offset + layout_index);
    }
...
inline CpuTensorView cpu_tensor_view(py::handle tensor, const char* name) {
    CPU& carrier = cpu_carrier_from_tensor(tensor, name);
...
inline void CPU::scatter(py::object to_scatter, py::object scatter_onto,
    ...
    CPU& destination_carrier = cpu_carrier_from_tensor(scatter_onto, "scatter_onto");
    if (&destination_carrier != this) {
        throw py::value_error("scatter_onto must be backed by this carrier");
    }
    if (!is_mutable()) {
        throw std::runtime_error("Carrier is not mutable");
    }
    ...
    auto& mapping_cache =
        py::cast<strideweave::layout_index::LayoutCache&>(mapping_cache_owner);
...
inline void require_two_mode_tensor(py::handle tensor, const char* name) {
    cpu_carrier_from_tensor(tensor, name);
...
inline py::object make_tensor(py::object carrier, py::object layout) {
    return tensor_type()(std::move(carrier), py::int_(0), std::move(layout));
}

struct CpuTensorAllocation {
    py::object carrier_object;
