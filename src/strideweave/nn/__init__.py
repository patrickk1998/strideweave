"""PyTorch-style neural network modules composed from strideweave primitives.

``strideweave.nn`` layers keep the core carriers untouched: every module is
...
``import strideweave.nn as nn`` — so the top-level ``strideweave`` namespace stays
