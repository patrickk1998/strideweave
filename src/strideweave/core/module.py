"""Module and parameter abstractions for StrideWeave models."""
...
class Parameter(Tensor):
    ...
    Args:
        tensor_or_carrier: Existing Tensor to wrap as a Parameter, or carrier
            object for direct Tensor construction.
        offset: Carrier offset used when constructing from a carrier.
        layout: Layout used when constructing from a carrier.
    ...
    Returns:
        Parameter instance backed by the provided tensor storage or carrier, with
    ...
    def __init__(
        self,
        tensor_or_carrier: Tensor | Any,
        offset: int | None = None,
        layout: Any | None = None,
        *,
        name: str | None = None,
    ) -> None:
        if isinstance(tensor_or_carrier, Tensor):
            ...
            super().__init__(
                tensor_or_carrier.carrier,
                tensor_or_carrier.offset,
                tensor_or_carrier.layout,
            )
        ...
        if offset is None or layout is None:
            raise TypeError(
                "Parameter requires offset and layout when constructed from a carrier"
            )
        super().__init__(tensor_or_carrier, offset, layout)
        self.name = _validate_name(name)


class Module:
    """Base class for StrideWeave modules with parameter registration.
