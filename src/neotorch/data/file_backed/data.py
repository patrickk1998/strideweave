...
def _session_directory() -> Path:
    ...
    if _session_directory_path is None:
        path = Path(tempfile.gettempdir()) / f".neotorch-filebacked-{uuid4().hex}"
...
def _validate_file_backed_dtype(dtype: DataType) -> DataType:
    if not isinstance(dtype, DataType):
        raise TypeError("FileBacked dtype must be a DataType")
    if dtype not in _STRUCT_FORMATS:
        raise ValueError(
            "FileBacked dtype must be DataType.Floating, DataType.Float32, or"
            " DataType.Int32"
        )
    return dtype
...
class FileBacked(Data):
    """Data storage backed by a raw binary file on disk.

    Values live in a file inside a hidden per-process directory under the
    system temporary directory. The file is allocated when the data object is
    created and removed when the data object is deleted; the whole directory
    ...
    moved to another data class with ``neotorch.move``.
    ...
    Args:
        ...
        dtype: Numeric element type; one of ``DataType.Floating``,
            ``DataType.Float32``, or ``DataType.Int32``.

    Examples:
        >>> from neotorch import FileBacked
        >>> data = FileBacked("weights.bin")
        >>> data.size()
        0
    """
        ...
        *,
        ...
        if type(self) is not FileBacked:
            raise NotImplementedError(
                "FileBacked data factory only supports exact FileBacked data"
            )
        result = FileBacked(
    ...
    def scatter(
        ...
        raise NotImplementedError("FileBacked data does not support scatter")
    ...
    def _write_value(self, index: int, value: Any) -> None:
        path = self._require_file()
        normalized = self._validate_index(index)
        if self._dtype is DataType.Int32:
