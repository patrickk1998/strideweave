...
class EvictableOperation(Operation):
    """Delegate one autograd operation to a promoted primary backend.
    ...
    Args:
        primary_operation: Fresh operation returned by the primary data class.

    Returns:
        Single-use forward adapter that remains reusable for backward.

    Examples:
        >>> from neotorch import CPU, EvictableOperation
    ...
    def __init__(self, primary_operation: Any) -> None:
        ...
        self._output_data: Evictable | None = None

    @property
    def primary_operation(self) -> Any:
        """Return the owned primary-backend operation.
    ...
    def _lower_tensor(tensor: Any) -> Any:
        ...
        if not isinstance(tensor.data, Evictable):
            raise TypeError("EvictableOperation requires Evictable tensor inputs")
        primary = tensor.data._require_promoted()
    ...
    def _require_compatible(inputs: tuple[Any, ...]) -> Evictable:
        ...
        data_inputs = [
            value.data
            for value in inputs
            if isinstance(value, Tensor) and isinstance(value.data, Evictable)
        ]
        if not data_inputs:
        ...
        for data in data_inputs[1:]:
            if type(data._require_promoted()) is not primary_class:
                raise TypeError("Evictable primary data classes must match")
            if type(data.secondary) is not secondary_class:
                raise TypeError("Evictable secondary data classes must match")
        return first
    ...
    def _forward(self, *inputs: Any) -> Any:
        ...
        wrapped_data = hierarchy._wrap_primary(primary_result.data)
        self._output_data = wrapped_data
        return Tensor(wrapped_data, primary_result.offset, primary_result.layout)
    ...
    def backward(self, gradient: Any) -> tuple[Any, ...]:
        ...
        if not self._forward_complete or self._output_data is None:
        ...
        expected_primary_class = type(self._output_data.primary)
        if isinstance(gradient.data, Evictable):
            lowered_gradient = self._lower_tensor(gradient)
        else:
            if type(gradient.data) is not expected_primary_class:
                raise TypeError("gradient data class must match the primary backend")
        ...
        for original, primary_gradient in zip(
            ...
            wrapped_data = original.data._wrap_primary(primary_gradient.data)
