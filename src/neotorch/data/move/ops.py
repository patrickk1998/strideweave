"""Cross-data-class move operations and their dispatch registry."""
...
from ..cpu import CPU
from ..file_backed.data import FileBacked
...
_move = import_module("neotorch._move")
...
class MoveOperation(Operation):
    """Base class for moving a tensor's values into another data instance.

    Forward validates the move, fills the destination data object through the
    ``_copy`` hook, and releases the source data. The destination dtype must
    match the tensor dtype. Concrete subclasses implement ``_copy`` and may
    pin ``source_class``/``destination_class`` to the data class pair they
    support. Backward moves the gradient back into the source data class, so
    gradients flow across data class boundaries.

    Backward materializes the gradient in the *released* source data class
    through ``new_like``, so every data class must keep ``new_like`` usable
    ...
    def _forward(self, tensor: Any, destination: Any) -> Any:
        ...
        if destination is tensor.data:
            raise ValueError("destination must not be the tensor's own data")
        if destination.is_released():
            raise RuntimeError("destination data is released")
        if not destination.is_mutable():
            raise RuntimeError("destination data must be mutable")
        if destination.type() is not tensor.data.type():
            raise TypeError("destination dtype must match the tensor dtype")
        if self.source_class is not None and type(tensor.data) is not self.source_class:
            raise TypeError(
        ...
        if destination.size() < required_size:
            raise ValueError("destination data is too small for the tensor layout")
...
class ElementwiseMoveOperation(MoveOperation):
    """Move between any data class pair by copying logical values one by one.

    Layout holes keep the destination's prior values. Used as the dispatch
    fallback for data class pairs without a registered move operation.
    """
...
class CpuToFileBackedMoveOperation(MoveOperation):
    """Move a CPU tensor into FileBacked data with a native bulk byte copy.
    ...
    def _copy(
        ...
        _copy_memory_to_file(
            ...
            tensor.data.pointer() + tensor.offset * itemsize,
            element_count * itemsize,
        )


class FileBackedToCpuMoveOperation(MoveOperation):
    """Move a FileBacked tensor into CPU data with a native bulk byte copy.
    ...
    def _copy(
        ...
        itemsize = tensor.data._itemsize
        _copy_file_to_memory(
            str(tensor.data.path),
...
def register_move_operation(
    ...
    """Register a concrete move operation for a data class pair.

    Args:
        source_class: Data class of the tensor being moved.
        destination_class: Data class of the destination instance.
    ...
    if not (isinstance(source_class, type) and issubclass(source_class, Data)):
        raise TypeError("source_class must be a Data subclass")
    if not (
        isinstance(destination_class, type) and issubclass(destination_class, Data)
    ):
        raise TypeError("destination_class must be a Data subclass")
    if not (
...
def unregister_move_operation(
    ...
    """Remove the registered move operation for a data class pair.
...
def registered_move_operation(
    ...
    when the block raises. Useful for tests and temporary backend overrides.
    ...
    Examples:
        ...
        ...     neotorch.move(tensor, Generic([0.0, 0.0]))
    """
...
def dispatch_move(source_class: type, destination_class: type) -> type[MoveOperation]:
    """Return the move operation class for a data class pair.
