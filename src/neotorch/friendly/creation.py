"""Ergonomic tensor factories over the CPU data backend.
...
data backends are still constructed directly from primitives.
"""
...
from ..data.dtype import DataType
...
def _new_cpu_tensor(layout: Layout, dtype: DataType) -> Tensor:
...
def _fill_logical(tensor: Tensor, values: list[Any]) -> None:
    ...
    for i, value in enumerate(values):
        tensor.data[tensor.offset + layout.index(i)] = value
...
def tensor(
    ...
    *,
    ...
    Args:
        ...
        dtype: CPU data type for the storage, ``Float32`` or ``Int32``.

    Returns:
        Tensor over fresh CPU storage holding the values.

    Examples:
        >>> from neotorch.friendly import tensor
    ...
    for value in flattened:
        result.data[result.offset + layout.index(tuple(coordinate))] = value
...
def full(*extents: int, value: float, dtype: DataType = DataType.Float32) -> Tensor:
    ...
    Examples:
        >>> from neotorch.friendly import full
...
def zeros(*extents: int, dtype: DataType = DataType.Float32) -> Tensor:
    ...
    Examples:
        >>> from neotorch.friendly import zeros
        >>> zeros(2, 3)[1, 2]
        0.0
    """
...
def ones(*extents: int, dtype: DataType = DataType.Float32) -> Tensor:
    ...
    Examples:
        >>> from neotorch.friendly import ones
        >>> ones(3)[2]
        1.0
    """

    value = 1 if dtype is DataType.Int32 else 1.0
    return full(*extents, value=value, dtype=dtype)


def arange(count: int, *, dtype: DataType = DataType.Float32) -> Tensor:
    ...
    Examples:
        >>> from neotorch.friendly import arange
        >>> arange(4)[3]
        3.0
    """
    ...
    values = (
        range(count) if dtype is DataType.Int32 else [float(i) for i in range(count)]
    )
...
def rand(*extents: int, rng: random.Random | None = None) -> Tensor:
    ...
    Examples:
        >>> import random
        >>> from neotorch.friendly import rand
        >>> 0.0 <= rand(2, 2, rng=random.Random(0))[0, 0] < 1.0
        True
    """

    rng = rng if rng is not None else random.Random()
    result = _new_cpu_tensor(column_major(*extents), DataType.Float32)
...
def randn(*extents: int, rng: random.Random | None = None) -> Tensor:
    ...
    Examples:
        >>> import random
        >>> from neotorch.friendly import randn
