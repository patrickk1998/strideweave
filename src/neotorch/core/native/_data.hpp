...
public:
    ...
    OwnershipToken claim_ownership() {
        ...
        if (is_owned()) {
            throw std::runtime_error("Data is already owned by another data object");
        }
    ...
    void relinquish_ownership(OwnershipToken token) {
        ...
        if (owner_access_depth_ != 0) {
            throw std::runtime_error(
                "Data ownership cannot be relinquished during owner access"
            );
        }
        ownership_token_ = 0;
    }
    ...
    void begin_owner_access(OwnershipToken token) {
        ...
        if (owner_access_depth_ != 0 && owner_access_thread_ != current_thread) {
            throw std::runtime_error(
                "Data is already being accessed by its owner on another thread"
            );
        }
    ...
    void end_owner_access(OwnershipToken token) {
        ...
        if (!has_owner_access()) {
            throw std::runtime_error(
                "Data owner access must end on the thread where it began"
            );
        }
        --owner_access_depth_;
        if (owner_access_depth_ == 0) {
...
protected:
    ...
    // applies the ownership guard and must not be overridden by backends.
...
private:
    ...
    void require_owner(OwnershipToken token) const {
        if (!is_owned() || ownership_token_ != token) {
            throw std::runtime_error("Invalid data ownership token");
        }
    }
    ...
    void require_external_or_owner_access() const {
        if (is_owned() && !has_owner_access()) {
            throw std::runtime_error(
                "Data is owned by another data object and cannot be modified directly"
            );
        }
    }
