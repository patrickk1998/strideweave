"""Optimizers for updating neotorch parameters in place.
...
class SGD:
    ...
    through each parameter's layout. Updates are in-place data writes, so
        ...
        with no_grad():
            for parameter in self._parameters:
                ...
                for i in range(parameter.layout.size):
                    ...
                    gradient_value = gradient.data[
                        gradient.offset + gradient.layout.index(i)
                    ]
                    parameter.data[physical] = (
                        parameter.data[physical] - self.lr * gradient_value
                    )
