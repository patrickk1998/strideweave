...
| ID | Invariant and canonical form | Enforcement | Evidence |
| --- | --- | --- | --- |
| `RT001` | Operation dispatch is invoked on a carrier instance and follows the documented exact carrier class. Every dispatch returns a fresh operation instance. | Test | `tests/test_carrier.py::test_generic_data_dispatch_op_returns_supported_operations`; `tests/test_carrier.py::test_data_operation_dispatch_is_instance_only`; `tests/test_cpu.py::test_cpu_dispatch_op_returns_supported_operations`; evictable adapter tests |
